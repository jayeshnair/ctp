...
if ( ! isset( $show_counter ) || ! $show_counter ) {
    $show_counter = yit_get_option( 'shop-category-count' );
}
// control for the discovery text
if ( ! isset( $discovery_text ) || ! $discovery_text ) {
    $discovery_text = yit_get_option( 'discovery-text' );
}
// control for the category's style
if ( ! isset( $style ) || ! $style ) {
    $style = yit_get_option( 'shop-category-style' );
}
// Put the style in the <li> tag
if ( $style == 'white' ): $woocommerce_loop['li_class'][] = 'white';
else: $woocommerce_loop['li_class'][] = 'black'; endif;
...
<li <?php post_class( $woocommerce_loop['li_class'] ); ?>>
    ...
    $default_cat_img = wc_placeholder_img_src();
