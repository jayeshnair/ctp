...
class YIT_Contact_Form {
    ...
    protected function _sendEmail()
    {
    	...
    	if ( isset( $_POST['yit_action'] ) AND ( $_POST['yit_action'] == 'sendemail' OR $_POST['yit_action'] == 'sendemail-ajax' ) )
    	{
            ...
            foreach( $fields as $field) {
                if( $field['type'] == 'checkbox' && ( array_key_exists( 'required', $field ) && $field['required'] == '1' ) && ! isset( $_POST['yit_contact'][ $field['data_name'] ])){
    ...
    public function module( $name_or_id, $action, $echo = true )
    {
    	...
    	foreach( $fields as $id => $field )
    	{
			...
			*/
    		...
    		switch( $field['type'] )
    		{
    			...
    			// select
    			case 'select':
    				$html .= "<label>" . stripslashes_deep( $field['title'], 'highlight-text' ) . "</label><select name=\"yit_contact[" . $field['data_name'] . "]\" id=\"" . $field['data_name'] . "-$form_name\" class=\"" . implode( $input_class, ' ' ) . "\">\n";
    			...
    			// password
    			case 'password':
    				$html .= "<label>" . stripslashes_deep( $field['title'], 'highlight-text' ) . "</label><input type=\"password\" name=\"yit_contact[{$field['data_name']}]\" id=\"{$field['data_name']}-$form_name\" class=\"" . implode( $input_class, ' ' ) . "\" value=\"$value\" />";
    			break;

    			// file
    			case 'file':
    				$html .= "<label>" . stripslashes_deep( $field['title'], 'highlight-text' ) . "</label><input type=\"file\" name=\"yit_contact[{$field['data_name']}]\" id=\"{$field['data_name']}-$form_name\" class=\"" . implode( $input_class, ' ' ) . "\" />";
    			break;
    		}
    	...
    	$html .= "\t\t\t\t<input type=\"submit\" name=\"yit_sendemail\" value=\"" . $this->get('submit_label') . "\" class=\"button " . $this->get('submit_alignment') . "\" />";
        $html .= "\t\t\t\t<div class=\"clear\"></div>";
    	$html .= "\t\t\t</li>\n";
