...
function yit_add_my_account_endpoint() {
    WC()->query->query_vars['change-password'] = 'change-password';
}
...
/* === HOOKS === */
add_action( 'wp_enqueue_scripts', 'yit_enqueue_woocommerce_assets', 11 );
add_action( 'yit_quick_view_load_static', 'yit_quick_view_load_static' );
...
add_filter( 'woocommerce_add_to_cart_url', 'yit_fix_add_to_cart_url' );
...
add_action( 'woocommerce_after_shop_loop_item', 'yit_product_other_actions', 8 );

// Add the rating's html
//add_action('woocommerce_single_product_summary','yit_rating_singleproduct', 10);

add_action( 'woocommerce_after_shop_loop_item', 'yit_shop_grid_description', 6 );
...
// First the category's thumb was managed as an <img> now is a background. so we remove the old hook
...
add_action( 'woocommerce_single_product_summary', 'yit_product_close_quick_view', 2 );
add_action( 'woocommerce_single_product_summary', 'yit_product_other_actions', 3 );
...
function yit_enqueue_woocommerce_assets() {
    ...
    wp_localize_script( 'yit-woocommerce', 'yit_woocommerce', array(
        ...
        'load_gif' => YIT_THEME_ASSETS_URL . '/images/ajax-loader.gif'
    ) );

    //dequeue default magnifier slider
...
function yit_quick_view_load_static() {

    global $woocommerce, $post;
    if ( ! yit_get_option( 'shop-use-quick-view' ) ) {
        return;
    }
    ...
    // assets for quick view
    wp_enqueue_script( 'modernizer-custom', YIT_THEME_ASSETS_URL . '/js/modernizr.custom.js', array( 'jquery', 'yit-commonlibraries' ), '', true );
    wp_enqueue_script( 'shop-quick-view', YIT_THEME_ASSETS_URL . '/js/quick-view.js', array( 'jquery', 'yit-commonlibraries' ), '', true );

    //add to cart, add to cart variation
    if ( ! wp_script_is( 'wc-add-to-cart-variation', 'registered' ) ) {
        wp_register_script( 'wc-add-to-cart-variation', $woocommerce->plugin_url() . '/assets/js/frontend/' . 'add-to-cart-variation.js', array( 'jquery' ), $woocommerce->version, true );
    }
...
function yit_shop_rating() {
    ...
    if ( yit_get_option( 'shop-view-show-rating' ) ) {
        echo '<div class="classic-rating">' . $product->get_rating_html() . '</div>';
    }
}
...
function yit_product_close_quick_view() {
    yith_wc_get_template( 'single-product/close-quick-view.php' );
}
...
function yit_fix_add_to_cart_url( $url ) {
    return remove_query_arg( 'doing_ajax', $url );
}
...
function woocommerce_template_loop_product_thumbnail() {
    global $product, $is_IE, $yit_products_is_slider;

    $attachments         = $product->get_gallery_attachment_ids();
    $active_hover        = ( bool ) ( ( yit_get_option( 'active-flip-3d' ) && ! $is_IE ) && ! empty( $attachments ) && isset( $attachments[0] ) );
    $quick_view          = '<span class="quick-view" data-product_id="' . $product->id . '"><span><i></i>' . yit_get_option( 'shop-quick-view-text' ) . '</span></span>';
    $can_show_quick_view = $yit_products_is_slider ? false : true;


    echo '<div class="thumb-wrapper">';

    echo '<a href="' . get_permalink() . '" class="thumb' . ( $active_hover ? ' flip' : ' fade' ) . '"><span class="face">' . woocommerce_get_product_thumbnail() . '</span>';
    ...
    if ( $can_show_quick_view && yit_get_option( 'shop-use-quick-view' ) && $active_hover ) {
        echo $quick_view;
    }
    echo '</span>';
...
function yit_woocommerce_checkout_registration_redirect( $location ) {
    if ( function_exists( 'wc_get_page_id' ) ) {
        if ( isset( $_POST['register'] ) && $_POST['register'] && isset( $_POST['yit_checkout'] ) && $location == get_permalink( wc_get_page_id( 'myaccount' ) ) ) {
            $location = get_permalink( wc_get_page_id( 'checkout' ) );
        }
    }
    else {
...
function yit_detect_span_catalog_image() {
    ...
    $product_width = yit_shop_catalog_w() + 6 + 2; // 10 = padding & 2 = border
    $is_span       = false;
...
function yit_size_images_style() {
    ...
    <style type="text/css">
        ul.products li.product.list .product-wrapper {
            padding-left: <?php echo yit_shop_catalog_w() + 30 + 7 + 2; ?>px;
            min-height: <?php echo yit_shop_catalog_h() + 7 + 2; ?>px;
        }

        ul.products li.product.list .product-wrapper .thumb-wrapper {
            margin-left: -<?php echo yit_shop_catalog_w() + 30 + 7 + 2; ?>px;
            width: <?php echo yit_shop_catalog_w()+10 ?>px;
            height: <?php echo yit_shop_catalog_h()+8 ?>px;
        }

        ul.products li.product.list .product-wrapper img.onsale {
            left: <?php echo yit_shop_catalog_w() + 3 - 63; ?>px;
        }

        .og-expander.woocommerce div.product div.images div.thumbnails {
            height: <?php echo yit_shop_thumbnail_h() + 10 ?>px;
        }
        ...
        .single-product.woocommerce div.product.product-layout-1 div.summary {
            ...
            padding-bottom: <?php echo yit_shop_thumbnail_h() ?>px;
        }
    </style>
<?php
}
...
function yit_fix_wishlist() {
    $options['general_settings']['5']['default'] = "shortcode";
}

apply_filters( 'yith_wcwl_tab_options', 'yit_fix_wishlist' );
...
/* Removed for WooCommerce integrity
...
function yit_get_current_cart_info() {
    ...
    $cart_icon = yit_get_option( 'minicart-icon' );
    return array(
        $items,
        $subtotal,
        $cart_icon,
...
function yit_add_to_cart_success_ajax( $datas ) {
    ...
    if ( isset( $_REQUEST['product_id'] ) ):
        ...
        <p><i></i><?php _e( 'You added', 'yit' ) ?>
            <strong>"<?php echo get_the_title( $_REQUEST['product_id'] ) ?>"</strong> <?php _e( 'to your cart', 'yit' ) ?>
        </p>
    <?php else: ?>
        <p><i></i><?php _e( 'You added', 'yit' ) ?> <?php _e( 'to your cart', 'yit' ) ?></p>
    <?php endif ?>
    <div class="actions">
        <a class="black-button continue-shopping" href="#"><?php _e( 'Continue shopping', 'yit' ) ?></a>
        <a class="red-button view-cart" href="<?php echo $woocommerce->cart->get_cart_url(); ?>"><?php _e( 'View cart', 'yit' ) ?></a>
        <!--<a class="button view-checkout" href="<?php echo $woocommerce->cart->get_checkout_url(); ?>"><?php _e( 'Checkout', 'yit' ) ?></a>-->
    </div>
...
function yit_woocommerce_default_image_dimensions() {
    ...
    if ( get_option( $field ) == false ) {
        ...
        update_option( 'shop_single_image_size', array( 'width' => 470, 'height' => 626, 'crop' => true ) );
        update_option( 'shop_catalog_image_size', array( 'width' => 262, 'height' => 307, 'crop' => true ) );
...
function yit_change_single_width( $size = array() ) {
    ...
    if ( isset( $post->ID ) && get_post_meta( $post->ID, 'shop_single_layout_2_width', true ) ) {
        $size['width'] = get_post_meta( $post->ID, 'shop_single_layout_2_width', true );
    }
    if ( isset( $post->ID ) && get_post_meta( $post->ID, 'shop_single_layout_2_height', true ) ) {
        $size['height'] = get_post_meta( $post->ID, 'shop_single_layout_2_height', true );
    }
    return $size;
}

// Show the message for log in in the checkout page
function yit_woocommerce_loginfobox() {
    $info_message = apply_filters( 'woocommerce_checkout_login_message', __( 'Returning customer?', 'yit' ) );
    echo    "<div class='row-fluid'><div class='log-info-container'>
                <p class='log-info-box'>" . esc_html( $info_message ) .
        "<a href='#' class='showlogin'> " . __( 'Click here to login', 'yit' ) . "</a></p>
            </div>";
}


/* Compare: Remove options Link or Button  */
function yith_woocompare_tab_options_std( $options ) {
...
add_filter( 'yith_woocompare_tab_options', 'yith_woocompare_tab_options_std' );
    ...
    function yith_wc_get_page_id( $page ) {
        ...
        if ( version_compare( preg_replace( '/-beta-([0-9]+)/', '', $woocommerce->version ), '2.1', '<' ) ) {
            ...
            if ( $page == 'pay' || $page == 'thanks' ) {
                $wc_order = ( version_compare( preg_replace( '/-beta-([0-9]+)/', '', $woocommerce->version ), '2.2', '<' ) ? new WC_Order() : wc_get_order() );
