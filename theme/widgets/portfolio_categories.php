	        		...
	        		<?php if (isset($cats) && $cats != '') : ?>
						<?php foreach( $cats as $cat=>$name ): ?>
				         	<?php if( yit_work_items_in_category($cat, $portfolio->ID) ): ?>
				           		<li class="segment-<?php echo ++$i ?>"><a href="<?php echo add_query_arg('cat', $cat, get_post_permalink($portfolio->ID)) ?>" data-value="<?php echo strtolower(preg_replace('/\s+/', '-', $cat)) ?>"><?php echo $name ?></a></li>
							<?php endif ?>
						<?php endforeach ?>
					<?php endif ?>
				</ul>
