	...
	<div class="row">
		<div class="span12">
			<ul class="filters">
				...
				<li class="all active"><a class="active all" href="<?php echo add_query_arg('cat', '') ?>" data-option-value="*"><?php _e('All Categories', 'yit') ?></a></li>
				<?php if(!empty($cats)): ?>
		...
		<div class="row">
            <ul id="portfolio" class="<?php echo $portfolio_type; ?> thumbnails">
            <?php
                $i = 1;
            ...
            <li <?php yit_work_class( $classes ) ?>>
				...
				<?php if ( !$lightbox ) : ?><a class="no-lightbox" href="<?php echo $post_permalink ?>" title=""><?php endif; ?>
					...
					<div class="ch-item<?php if ( $lightbox || $details || $title ) : ?> ch-item-hover<?php endif ?>" style="background: url('<?php yit_image( "id=$image_id&size=thumb_portfolio_4cols&output=url" ); ?>') no-repeat center;">
						...
						<div class="ch-info">
							...
							<div class="ch-info-icons<?php echo !yit_work_get('title') ? ' ch-info-icons-no-title' : '' ?>">
