...
if( !function_exists('yit_header_fixed') ) {
    function yit_header_fixed($classes) {
        if( yit_get_option('fixed-header') ) {
...
if( !function_exists('yit_wpml_nav_menu') ) {
    function yit_wpml_nav_menu( $nav_menu ) {

        if( preg_match('/<a href="#">\{WPML\}<\/a>/', $nav_menu, $output_array) ) {
            ob_start(); do_action('icl_language_selector'); $wpml = ob_get_clean();
            $nav_menu = str_replace($output_array[0], $wpml, $nav_menu);
        }
...
if( !function_exists('yit_myaccount_nav_menu') ) {
    function yit_myaccount_nav_menu( $nav_menu ) {
        if( preg_match('/<a href=\"([^\"]*)\">\{MYACCOUNT\}<\/a>/', $nav_menu, $output_array) ) {
            ob_start();

            echo '<a href="' . $output_array[1] . '">';

            if( is_user_logged_in() ) {
                _e('My Account', 'yit');
            } else {
                _e('Login-Register', 'yit');
            }

            echo '</a>';

            $myaccount = ob_get_clean();
            $nav_menu = str_replace($output_array[0], $myaccount, $nav_menu);
        }
...
if( !function_exists('yit_walker_nav_menu_title') ) {
    function yit_walker_nav_menu_title( $title ) {
        return '';
    }
}
