...
<div class="cart_totals <?php if ( WC()->customer->has_calculated_shipping() ) {
    ...
    <table cellspacing="0">
        <thead>
        <tr>
            <th colspan="2" class="head"><?php _e( 'Shopping Bag Totals', 'yit' ) ?></th>
        </tr>
        </thead>
        ...
        <p>
            <small><?php
                ...
                $estimated_text = WC()->customer->is_customer_outside_base() && ! WC()->customer->has_calculated_shipping()
                    ? sprintf( ' ' . __( ' (taxes estimated for %s %s)', 'yit' ), WC()->countries->estimated_for_prefix(), WC()->countries->countries[WC()->countries->get_base_country()] )
                    : '';
    ...
    <div class="cart_update_checkout">
        <input type="submit" class="button" name="update_cart" value="<?php _e( 'Update Shopping Cart', 'yit' ); ?>" />
        <input type="submit" class="checkout-button button alt" name="proceed" value="<?php _e( 'Proceed to Checkout', 'yit' ); ?>" />
